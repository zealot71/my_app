...
userSchema.pre('save', function(next) { // save() 하기 전에 할 행동 설정
  var user = this;
...
app.use(bodyParser.json()); // body json으로 파싱
app.use(bodyParser.urlencoded({extended:true}));  // url 데이터를 받기 위해
...
passport.deserializeUser(function(id, done) { // session 으로부터 개체를 어떻게 가져올지 설정 ( id를 넘겨받아 DB에서 user를 찾고, user를 가져옴 )
